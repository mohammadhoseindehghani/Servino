...
public class SuggestionAppService(ISuggestionService suggestionService,IRequestService requestService,
    ILogger<SuggestionAppService> logger) : ISuggestionAppService
{
    public async Task<Result<bool>> CreateAsync(
          CreateSuggestionDto command, CancellationToken ct)
    {
        ...
        var check = await suggestionService
            .IsExpertSendSuggestionBeforeAsync(
                command.ExpertId, command.RequestId, ct);

        if (check)
            return Result<bool>.Failure("برای این درخواست قبلا پیشنهاد ارسال کردید.");

        var basePrice =
            await requestService.GetBasePriceByRequestIdAsync(
                command.RequestId, ct);
    ...
    public async Task<List<SuggestionSummaryDto>> GetByRequestIdAsync(
    ...
    public async Task<Result<SuggestionDto>> GetByIdAsync(
